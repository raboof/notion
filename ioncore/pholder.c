...
{
    bool ret=FALSE;
    CALL_DYN_RET(ret, bool, pholder_do_attach, ph, 
                 (ph, hnd, hnd_param, flags));
    return ret;
}
...
bool pholder_attach(WPHolder *ph, WRegion *reg, int flags)
{
    if(ph->redirect!=NULL){
        return pholder_attach(ph->redirect, reg, flags);
    }else{
